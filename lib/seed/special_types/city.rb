Seed::Plant.register_type("city") do
  pre = %w( New Old Los Santa )
  
  # Shamelessly ripped from Ryan Davis's babel gem
  primary = %w(Smith Johnson Williams Jones Brown Davis Miller Wilson Moore Taylor Anderson Thomas Jackson White
  ...
  post = ["ville", "town", "sboro", " City"]
  
  "#{rand % 2 == 0 ? "pre[rand(3)] " : ''}#{primary[rand(primary.length - 1)]}#{post[rand(post.length - 1)]}"
end